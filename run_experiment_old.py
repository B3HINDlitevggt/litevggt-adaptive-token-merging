...
[모드 선택]
  ...
  sttm             : STTM 공간+시간 merge      (Exp 5) ← 새로 추가
...
CUDA_VISIBLE_DEVICES=0 python3 run_experiment.py \
    --img_dir ./SampleSet/test2 \
    --output_dir ./output/sttm_spatial_only \
    --gt_path "$GT_PATH" \
    --mode sttm \
    --sttm_temporal_thresh -1
"""
...
CUDA_VISIBLE_DEVICES=3 python3 run_experiment.py \
    ...
    --gt_path "$GT_PATH" \
    --mode sttm \
    --sttm_spatial_thresh 0.99 \
    --sttm_temporal_thresh 0.95
"""